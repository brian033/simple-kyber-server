...
var Price = /*#__PURE__*/function (_Fraction) {
  ...
  Price.fromReserves = function fromReserves(inputReserves, outputReserves) {
    !(inputReserves.length === outputReserves.length) ?  invariant(false, 'RESERVE')  : void 0;
    var prices = [];

    for (var _iterator2 = _createForOfIteratorHelperLoose(inputReserves.entries()), _step2; !(_step2 = _iterator2()).done;) {
      var _step2$value = _step2.value,
          i = _step2$value[0],
          inputReserve = _step2$value[1];
...
var Pair = /*#__PURE__*/function () {
  function Pair(address, tokenAmountA, tokenAmountB, virtualTokenAmountA, virtualTokenAmountB, fee, amp) {
    ...
    var virtualTokenAmounts = tokenAmountA.token.sortsBefore(tokenAmountB.token) // does safety checks
  ...
  _proto.priceOfReal = function priceOfReal(token) {
    !this.involvesToken(token) ?  invariant(false, 'TOKEN')  : void 0;
    return token.equals(this.token0) ? new Price(this.token0, this.token1, this.tokenAmounts[0].raw, this.tokenAmounts[1].raw) : new Price(this.token1, this.token0, this.tokenAmounts[1].raw, this.tokenAmounts[0].raw);
  }
  /**
  ...
  _proto.getOutputAmount = function getOutputAmount(inputAmount) {
    ...
    var outputToken = inputAmount.token.equals(this.token0) ? this.token1 : this.token0;
    var inputReserve = this.virtualReserveOf(inputAmount.token);
    var outputReserve = this.virtualReserveOf(outputToken);
    var inputAmountWithFee = JSBI.divide(JSBI.multiply(inputAmount.raw, JSBI.subtract(PRECISION, this.fee)), PRECISION);
    var numerator = JSBI.multiply(inputAmountWithFee, outputReserve.raw);
    var denominator = JSBI.add(inputReserve.raw, inputAmountWithFee);
    var outputAmount = new TokenAmount(outputToken, JSBI.divide(numerator, denominator));
  ...
  _proto.getInputAmount = function getInputAmount(outputAmount) {
    ...
    var inputToken = outputAmount.token.equals(this.token0) ? this.token1 : this.token0;
    ...
    var inputAmount = new TokenAmount(inputToken, JSBI.divide(JSBI.subtract(JSBI.add(numerator, denominator), ONE), denominator));
  ...
  _proto.getLiquidityValue = function getLiquidityValue(token, totalSupply, liquidity, feeBps, kLast) {
    ...
    if (JSBI.equal(feeBps, ZERO)) {
      ...
      if (!JSBI.equal(kLastParsed, ZERO)) {
        var rootK = sqrt(JSBI.multiply(this.virtualReserve0.raw, this.virtualReserve1.raw));
        var rootKLast = sqrt(kLastParsed);

        if (JSBI.greaterThan(rootK, rootKLast)) {
          var numerator = JSBI.multiply(JSBI.multiply(totalSupply.raw, JSBI.subtract(rootK, rootKLast)), feeBps);
  ...
  currentPairs, originalAmountIn, bestTrades) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (bestAmountOut.token.equals(tokenOut)) {
        sortedInsert(bestTrades, new Trade(new Route([].concat(currentPairs, [bestPool]), originalAmountIn.currency, currencyOut), originalAmountIn, exports.TradeType.EXACT_INPUT), maxNumResults, tradeComparator);
      } else if (maxHops > 1 && pairs.length > 1) {
        var pairsExcludingThisPair = pairs.slice(0, i).concat(pairs.slice(i + 1, pairs.length)); // otherwise, consider all the other paths that lead from this token as long as we have not exceeded maxHops

        Trade.bestTradeExactIn(pairsExcludingThisPair, bestAmountOut, currencyOut, {
          maxNumResults: maxNumResults,
          maxHops: maxHops - 1
        }, [].concat(currentPairs, [bestPool]), originalAmountIn, bestTrades);
      }
    }
  ...
  currentPairs, originalAmountOut, bestTrades) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (bestAmountIn.token.equals(tokenIn)) {
        sortedInsert(bestTrades, new Trade(new Route([bestPool].concat(currentPairs), currencyIn, originalAmountOut.currency), originalAmountOut, exports.TradeType.EXACT_OUTPUT), maxNumResults, tradeComparator);
      } else if (maxHops > 1 && pairs.length > 1) {
        var pairsExcludingThisPair = pairs.slice(0, i).concat(pairs.slice(i + 1, pairs.length)); // otherwise, consider all the other paths that arrive at this token as long as we have not exceeded maxHops

        Trade.bestTradeExactOut(pairsExcludingThisPair, currencyIn, bestAmountIn, {
          maxNumResults: maxNumResults,
          maxHops: maxHops - 1
        }, [bestPool].concat(currentPairs), originalAmountOut, bestTrades);
      }
    }
          ...
          try {
            ...
            return Promise.resolve(poolContract.getTradeInfo()).then(function (_ref) {
              ...
              return Promise.resolve(poolContract.ampBps()).then(function (ampBps) {
                ...
                return new Pair(address, new TokenAmount(tokenA, balances[0]), new TokenAmount(tokenB, balances[1]), new TokenAmount(tokenA, balances[2]), new TokenAmount(tokenB, balances[3]), parseBigintIsh(feeInPrecision), JSBI.BigInt(ampBps));
              });
            });
          } catch (e) {
            return Promise.reject(e);
